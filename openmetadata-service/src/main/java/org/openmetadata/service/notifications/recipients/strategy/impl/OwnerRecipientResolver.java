  ...
  public Set<Recipient> resolve(
    ...
    try {
      ...
      if (Entity.THREAD.equalsIgnoreCase(entityType)) {
        return resolveOwnersFromThread(entityId, destination);
      }

      // Standard handling for other entities
      return resolveOwnersFromEntity(entityId, entityType, destination);
  ...
  private @NotNull Set<Recipient> resolveOwnersFromThread(
    ...
    if (thread.getEntityRef() != null) {
      try {
        ...
        LOG.debug("Failed to resolve parent entity owners for thread {}", entityId, e);
      }
    }
