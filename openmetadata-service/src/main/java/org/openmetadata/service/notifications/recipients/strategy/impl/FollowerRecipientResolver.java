...
public class FollowerRecipientResolver implements RecipientResolutionStrategy {
  ...
  public Set<Recipient> resolve(
    ...
    try {
      ...
      if (Entity.THREAD.equalsIgnoreCase(entityType)) {
        return resolveFollowersFromThread(entityId, destination);
      }

      // Standard handling for other entities
      return resolveFollowersFromEntity(entityId, entityType, destination);
  ...
  private @NotNull Set<Recipient> resolveFollowersFromEntity(
      UUID entityId, String entityType, SubscriptionDestination destination) {
  ...
  private @NotNull Set<Recipient> resolveFollowersFromThread(
      UUID entityId, SubscriptionDestination destination) {
    ...
    // For Announcement threads, resolve followers from the referenced entity
    EntityReference referencedEntity = thread.getEntityRef();
    return resolveFollowersFromEntity(
        referencedEntity.getId(), referencedEntity.getType(), destination);
  }
